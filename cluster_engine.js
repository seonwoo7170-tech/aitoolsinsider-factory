...
const PURE_PROMPT = "MANDATORY: Return ONLY article HTML content. NO intro. NO numbers/Part labels in headings. Use professional standalone guide tone.";
...
async function generateImage(prompt, kieKey, imgbbKey) {
    ...
    try {
        // REVERT TO WORKING OR CORRECT ENDPOINT
        const res = await axios.post("https://api.kie.ai/v1/image/generate", {
            prompt: cleanP + ", ultra high quality photography, cinematic",
            model: "z-image",
            width: 1024,
            height: 768
...
async function run() {
    console.log("🚀 VUE V16.9.3 Titan-Engine Initializing...");
    ...
    const bText = lang === 'ko' ? "타이탄 에디션" : "TITAN EDITION";
    const readBtn = lang === 'ko' ? "인사이트 정독하기 →" : "Read Full Story →";
    ...
    for (let i = 0; i < 5; i++) {
        ...
        console.log(`🎯 [${isMain ? 'MASTER' : 'SUB'}] Titan Article [${i+1}/5]...`);

        const titlePrompt = `Generate a professional standalone Long-tail SEO Title for: "${currentContext}". NO "Part X", NO numbers.`;
        const titleRaw = await callGemini(model, titlePrompt);
        const title = titleRaw.trim().replace(/[\\"]/g, '');

        const imgPs = (await callGemini(model, `Return 4 short descriptive image prompts for "${title}". English.`)).split("\n").slice(0, 4);
        const imageUrls = await Promise.all(imgPs.map(p => generateImage(p.trim(), kieKey, imgbbKey)));

        const sumRaw = await callGemini(model, `3 powerful summary bullets for "${title}" in ${lang}.`);
        ...
        let finalBody = STYLE + `<div class="vue-content-body">
            ...
            <div style="background:#f8fafc;border-radius:25px;padding:2.5rem;margin:4rem 0;border:2px dashed #6366f1;"><span style="font-weight:900;color:#4338ca;font-size:1.3rem;margin-bottom:1.5rem;display:block;">TITAN INSIGHTS</span>${cleanSum}</div>`;

        if (isMain) {
            const mBody = await callGemini(model, `Write Premium Cluster Pillar Article for "${targetTopic}" in ${lang}. NO numeric subheads. HTML ONLY.`);
            ...
            for(let p=1; p<=3; p++) {
                const pBody = await callGemini(model, `Deep Analysis Segment ${p} for article: "${title}" in ${lang}. NO numeric subheads. HTML ONLY.`);
        ...
        const faqR = await callGemini(model, `15 High-Intent SEO FAQs for "${title}" in ${lang}. Return ONLY raw JSON Array.`);
        try {
        ...
        finalBody += `<div style="margin-top:100px; text-align:center; color:#94a3b8; font-size:13px;">© VUE Titan-Engine.</div></div>`;
        ...
        try {
            ...
            subHistory.push({ title, url: res.data.url, summary: cleanSum });
            console.log(`✅ Posted: ${title}`);
        } catch (e) { 
            console.log("Post Error Body:", e.response ? JSON.stringify(e.response.data) : e.message);
        }
        
        await sleep(15000); 
        publishTime = new Date(publishTime.getTime() + (140 + Math.random() * 50) * 60 * 1000);
    }
}
run();