...
function cleanHtml(raw) {
    ...
    // SAFE REGEXP STRINGS FOR NODE.JS
    const stripBlocks = [
    ...
    // Heading Number Purge (Aggressive)
...
async function generateImage(prompt, kieKey, imgbbKey) {
    ...
    try {
        const res = await axios.post("https://api.kie.ai/v1/images/generations", {
            prompt: cleanP + ", ultra high quality photography, cinematic, detailed",
            n: 1,
            size: "1024x768"
        }, { headers: { "Authorization": "Bearer " + kieKey }, timeout: 60000 });
        
        let url = "";
        if (res.data && res.data.data && res.data.data[0]) url = res.data.data[0].url;
        else if (res.data && res.data.image_url) url = res.data.image_url;
...
async function run() {
    console.log("🚀 VUE V16.9.2 Solid-Engine Initializing...");
    ...
    const bText = lang === 'ko' ? "스페셜 에디션" : "SPECIAL EDITION";
    const readBtn = lang === 'ko' ? "인사이트 더보기 →" : "Read More →";
    ...
    for (let i = 0; i < 5; i++) {
        ...
        console.log(`🎯 [${isMain ? 'MASTER' : 'SUB'}] Solid Mode Article [${i+1}/5]...`);

        const titlePrompt = `Professional Long-tail SEO Title for: "${currentContext}". NO "Part X", NO numbers, NO colon. Standalone guide style.`;
        const titleRaw = await callGemini(model, titlePrompt);
        const title = titleRaw.trim().replace(/[\\"]/g, '');

        const imgPs = (await callGemini(model, `4 descriptive image prompts for "${title}". English.`)).split("\n").slice(0, 4);
        const imageUrls = await Promise.all(imgPs.map(p => generateImage(p.trim(), kieKey, imgbbKey)));

        const sumRaw = await callGemini(model, `3 key takeaways for "${title}" in ${lang}.`);
        ...
        if (isMain) {
            const mBody = await callGemini(model, `Flagship Article for "${targetTopic}" in ${lang}. NO numeric subheads. Use h2/h3. HTML ONLY.`);
            ...
            for(let p=1; p<=3; p++) {
                const pBody = await callGemini(model, `Deeper Analysis Section ${p} for article: "${title}" in ${lang}. NO numeric subheads. HTML ONLY.`);
        ...
        finalBody += `<div style="margin-top:100px; text-align:center; color:#94a3b8; font-size:13px;">© VUE Solid-Engine Platform.</div></div>`;
        ...
        await sleep(15000); 
        publishTime = new Date(publishTime.getTime() + (139 + Math.random() * 49) * 60 * 1000);
    }
}
run();