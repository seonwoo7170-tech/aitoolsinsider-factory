...
const STYLE = `<style>
  ...
  .vue-body h2 { font-size: 1.6rem; font-weight: bold; color: #000; background-color: #FFD8A8; padding: 12px 20px; border-radius: 8px; margin: 6rem 0 2.5rem; line-height: 1.25; }
  .vue-body h3 { font-size: 1.35rem; font-weight: 800; color: #1e293b; margin: 4rem 0 1.8rem; line-height: 1.4; }
  ...
  .vue-yt-line { background: #000; color: #fff; padding: 4px 18px; font-weight: 950; font-size: 2.3rem; line-height: 1.3; box-shadow: 10px 0 0 #000, -10px 0 0 #000; }
  .vue-yt-line.highlight { background: #6366f1; box-shadow: 10px 0 0 #6366f1, -10px 0 0 #6366f1; }
  .toc-container { background-color: #F8E8EE; padding: 25px; border-radius: 12px; border: 2px solid #000; max-width: 85%; margin: 40px auto; box-shadow: 0 0 15px rgba(255, 105, 180, 0.5); animation: fast-glow 0.7s infinite alternate, shake 3s infinite ease-in-out, float 2s infinite ease-in-out; transition: transform 0.3s, box-shadow 0.3s; }
  .toc-title { font-size: 22px; font-weight: bold; text-align: center; color: #000; display: block; margin-bottom: 20px; }
  .toc-container ul { list-style: none; padding: 0; }
  .toc-container li { margin-bottom: 12px; }
  .toc-container li a { text-decoration: none !important; font-weight: bold; color: #000 !important; font-size: 15px; display: block; transition: 0.2s; }
  .toc-container li a:hover { color: #6366f1 !important; transform: translateX(5px); }
  @keyframes fast-glow { 0% { box-shadow: 0 0 10px rgba(255, 105, 180, 0.4); } 100% { box-shadow: 0 0 20px rgba(255, 105, 180, 0.8); } }
  @keyframes shake { 0%, 100% { transform: translateX(0); } 25% { transform: translateX(-1px); } 75% { transform: translateX(1px); } }
  @keyframes float { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-5px); } }
  .alert-box { background-color:#fff3cd; border:1px solid #ffc107; border-radius:8px; padding:20px; margin:40px 0; font-size:14px; color:#856404; }
  .summary-box { background-color:#d4edda; border:1px solid #28a745; border-radius:8px; padding:20px; margin:40px 0; font-size:14px; color:#155724; }
...
async function genImg(p, k) { if(!k) return ""; try { const cr = await axios.post("https://api.kie.ai/api/v1/jobs/createTask", { model: "z-image", input: { prompt: p + " cinematic lighting, ultra detailed", aspect_ratio: "16:9" } }, { headers: { Authorization: "Bearer " + k.trim() } }); if(cr.data.code !== 200) return ""; const tid = cr.data.data.taskId; for(let a=0; a<20; a++) { await new Promise(res => setTimeout(res, 8000)); const pr = await axios.get("https://api.kie.ai/api/v1/jobs/recordInfo?taskId=" + tid, { headers: { Authorization: "Bearer " + k.trim() } }); if(pr.data.data.state === 'success') return JSON.parse(pr.data.data.resultJson).resultUrls?.[0] || ""; if(pr.data.data.state === 'fail') return ""; } } catch(e) {} return ""; }
...
async function run() {
  ...
  let bodyContent = STYLE + "<div class='vue-body'>" + 
    ...
    "<p data-ke-size='size16'>" + title + "과 관련된 모든 정보를 담았습니다. 지금 바로 확인해 보세요.</p>" + tocHtml + 
  ...
  for(let p=0; p < cls.length; p++) {
    ...
    const prompt = "Write SECTION " + (p+1) + " of " + cls.length + ".\nTITLE: " + cls[p].t + "\n[RULES]: NO headers. Use h3. Start with narrative in " + lang + ".";
  ...
  const alertTxt = lang === 'en' ? "NOTICE: This content provides professional insights but may not guarantee individual results." : "⚠️ 면책 문구: 본 글은 전문적인 정보를 제공하지만 모든 환경에서 완벽한 해결을 보장하지는 않습니다.";
  const summaryTxt = lang === 'en' ? "SUMMARY: Leverage these insights to dominate the 2026 landscape." : "📌 요약: 본 글에서 제시한 전략을 통해 여러분의 노트북 성능을 100% 활용해 보세요.";
  bodyContent += `<div class='alert-box'>${alertTxt}</div><div class='summary-box'>${summaryTxt}</div></div>`;
