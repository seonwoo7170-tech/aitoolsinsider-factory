...
const STYLE = `<style>
    ...
    .vue-main-thumb { position: relative; width: 100%; height: 480px; border-radius: 50px; overflow: hidden; margin-bottom: 5.5rem; box-shadow: 0 45px 100px -20px rgba(0,0,0,0.4); background: #f1f5f9; }
...
async function callAI(model, prompt, isHTML = false, retry = 3) {
    try {
        const r = await model.generateContent((isHTML ? "EEAT. Break text every 3-4 sentences with '<div class=\"vue-ad-slot\"></div>'. HTML.\n\n" : "RAW TEXT. ONE viral SEO title. NO colon.\n\n") + prompt);
...
async function genImg(p, k) {
    if (!k) return "";
    const cleanP = p.replace(/[^\w\s]/g, ' ').substring(0, 250).trim();
    try {
...
async function run() {
    console.log("💎 VUE Engine v1.3.1 Active.");
    ...
    let body = STYLE + `<div class="vue-content-body">` + 
        ...
        `<div style="background:#f8fafc;border-radius:45px;padding:3.8rem;margin:5.5rem 0;border:3px dashed #6366f1;"><span style="font-weight:1100;color:#4338ca;font-size:1.6rem;margin-bottom:2.2rem;display:block;">EXPERT SUMMARY</span><div style="color:#334155;line-height:2.4;">${cleanSum}</div></div><div class="vue-ad-slot"></div>`;

    for(let p=1; p<=4; p++) {
        const content = clean(await callAI(model, `Chapter ${p} for "${title}" in ${lang}. MIN 3000 chars. HTML.`, true));
        body += content;
    }
    ...
    body += `<div style="margin-top:100px;text-align:center;color:#94a3b8;font-size:14px;font-weight:800;">© VUE TITANIUM v1.3.1</div></div>`;
