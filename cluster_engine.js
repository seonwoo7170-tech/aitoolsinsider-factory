...
async function writeAndPost(model, target, lang, blogger, bId, pTime, extraLinks = [], idx, total, searchQuery = '') {
    ...
    if(extraLinks.length > 0) {
        ...
        clusterContext = `\\\\n[CLUSTER_LINKS] (메인 허브 전용): 아래 링크들은 현재 클러스터의 핵심 서브 글들입니다. 본문 마지막 결론(PART 2) 바로 직전에 [5-10](A) 프리미엄 버튼 형식으로 모두 배치하십시오.\\\\n${linkList}`;
    }
    let archiveContext = '';
    if (globalArchives && globalArchives.length > 0) {
        const archList = globalArchives.map(l => `- Title: ${l.title}, URL: ${l.url}`).join('\\\\n');
        archiveContext = `\\\\n[ARCHIVE_LINKS] (공통 아카이브): 아래 링크들은 블로그의 최근 글들입니다. 포스팅 최하단 마무리 직전에 [5-10](B) 텍스트 링크 목록(버튼 아님)으로 5개를 모두 삽입하십시오.\\\\n${archList}`;
    }
