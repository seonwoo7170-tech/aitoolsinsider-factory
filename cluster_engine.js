...
const STYLE = `<style>
    ...
    .vue-main-thumb { position: relative; width: 100%; height: 550px; border-radius: 50px; overflow: hidden; margin-bottom: 5.5rem; box-shadow: 0 45px 100px -20px rgba(0,0,0,0.4); background: #f1f5f9; }
...
async function genImg(p, k) {
    const cleanP = p.replace(/[^\w\s\uAC00-\uD7A3]/g, ' ').substring(0, 200).trim();
    
    // 🛡️ TIER 1: Kie.ai (High Quality)
    if (k && k.length > 5) {
        try {
            console.log(`🎨 [TIER 1] Requesting Kie.ai (3:2)...`);
            const r = await axios.post("https://api.kie.ai/api/v1/gpt4o-image/generate", { prompt: cleanP, n: 1, size: "3:2" }, { headers: { Authorization: "Bearer " + k }, timeout: 40000 });
            ...
            console.log(`⚠️ [TIER 1 FAIL] Status: ${e.response?.status} - ${e.response?.data?.msg || "Timeout"}`);
        }
    }

    // 🛡️ TIER 2: Pollinations.ai (FREE / GUARANTEED)
    console.log(`🎨 [TIER 2] Using FREE Miracle Engine (Pollinations)...`);
    const freeUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(cleanP)}?width=1024&height=768&nologo=true&seed=${Math.floor(Math.random()*9999)}`;
    console.log(`✅ [TIER 2 MIRACLE] 🔗 ${freeUrl}`);
    return freeUrl;
}
...
async function run() {
    console.log("💎 VUE Visual Miracle v1.2.3 Active.");
    ...
    for (let i = 0; i < 1; i++) {
        console.log(`🎯 [1/1] Visual Miracle Run: ${target}`);
        let titleRaw = await callAI(model, `Generate a High-Click Viral SEO Title for: "${target}" in ${lang}. Natural & Powerful. Plain text only.`, false);
        ...
        let body = STYLE + `<div class="vue-content-body">
            <div class="vue-main-thumb"><img src="${imgUrl}"><div style="position:absolute;inset:0;background:rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;padding:25px;"><div style="font-size:3.5rem;font-weight:1050;color:#fff;text-shadow:0 15px 70px rgba(0,0,0,1);text-align:center;line-height:1.2;">${title}</div></div></div>
        ...
        let context = "";
        for(let p=1; p<=4; p++) {
            const content = clean(await callAI(model, `Write Deep Chapter ${p} for "${title}" in ${lang}. ${context ? "PREVIOUS: " + context.substring(0, 800) : ""} MIN 3000 chars. Use <div class=\"vue-ad-slot\"></div> every 3-4 sentences. HTML.`, true));
            body += content;
        ...
        body += `<div style="margin-top:100px;padding:40px;background:#f8fafc;border-radius:35px;border:2px solid #e2e8f0;font-size:14px;color:#64748b;line-height:2;"><b>Expert Disclaimer:</b> AI-generated for educational purpose. © VUE MIRACLE v1.2.3</div></div>`;
        ...
        try {
            ...
            console.log(`✅ Miracle Success: ${title}`);
