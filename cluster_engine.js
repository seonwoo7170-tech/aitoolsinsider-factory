...
const STYLE = `<style>
    ...
    .vue-thumb-badge { background: #facc15; color: #000; padding: 8px 18px; border-radius: 8px; font-size: 0.9rem; font-weight: 950; margin-bottom: 20px; box-shadow: 0 10px 20px rgba(250, 204, 21, 0.3); }
    ...
    .vue-faq-section { background: #f8fafc; border-radius: 2rem; padding: 4rem; margin-top: 6rem; border: 1px solid #e2e8f0; }
    .vue-faq-header { font-size: 2rem; font-weight: 1000; color: #0f172a; margin-bottom: 3rem; text-align: center; border-bottom: 3px solid #6366f1; display: inline-block; padding-bottom: 10px; width: 100%; }
    ...
    .vue-faq-a { color: #334155; font-size: 1.1rem; line-height: 1.8; padding-left: 20px; margin: 0; }
...
function cleanHtml(raw) {
    ...
    clean = clean.replace(/<html[\\s\\S]*?>|<\\/html>|<head[\\s\\S]*?>|<\\/head>|<body[\\s\\S]*?>|<\\/body>|<title[\\s\\S]*?>|<\\/title>/gi, '');
    return clean.trim();
}
    ...
    for (let i = 0; i < retries; i++) {
        try {
            ...
            if (e.message.includes('429') && i < retries - 1) {
                console.log(`⚠️ Rate limit. Retry in ${(i + 1) * 10}s...`);
...
async function generateImage(prompt, kieKey, imgbbKey) {
    ...
    if (!finalUrl) {
        const safePrompt = encodeURIComponent(prompt.replace(/[^a-zA-Z0-9\\s]/g, ' '));
...
async function run() {
    console.log("🚀 VUE V16.4.2 Pro-Disclaimer Engine Launching...");
    ...
    // Revised: Professional Disclaimer (No AI mentions)
    const disclaimerText = lang === 'ko' 
        ? "본 콘텐츠에 포함된 정보는 일반적인 정보 제공을 목적으로 하며, 특정적 사안에 대한 법적, 전문적 조언을 대신할 수 없습니다. 정보의 정확성을 위해 노력하였으나 실제 적용 시 발생하는 결과에 대해 본 블로그는 어떠한 법적 책임도 지지 않음을 알려드립니다. 모든 결정은 본인의 판단에 따라 신중하게 결정하시기 바랍니다."
        : "The information provided in this publication is for general informational and educational purposes only and is not intended as professional advice. While we endeavor to provide accurate content, we make no representations or warranties of any kind regarding its completeness. Any reliance you place on such information is strictly at your own risk.";

    let publishTime = new Date(Date.now() + 15 * 60 * 1000);

    for (let i = 0; i < 5; i++) {
        const isMain = (i === 4);
        console.log(`📂 Article [${i+1}/5] Final Processing...`);
        ...
        const imgPrompts = imgPromptRaw.split("\n").filter(p => p.trim() && p.length > 10).slice(0, 4);
        const imageUrls = await Promise.all(imgPrompts.map(p => generateImage(p.trim() + ", 8k, professional", kieKey, imgbbKey)));
        ...
        if (isMain) {
            ...
            let sectionContent = "";
            for(let p=1; p<=3; p++) {
                const chunkRaw = await callGemini(model, `Section ${p} for "${title}" in ${lang}. HTML fragments.`);
        ...
        const faqRaw = await callGemini(model, `Generate 15-20 highly technical FAQs for "${title}" in ${lang}. Output JSON: [{"q":"...","a":"..."}].`);
        try {
        ...
        // [v16.4.2 Professional Disclaimer]
        finalBody += `<div class="vue-disclaimer-section"><p class="vue-disclaimer-text">${disclaimerText}</p></div>`;
        finalBody += `</div>`;
