...
const STYLE = `<style>
    ...
    .vue-content-body h2 { font-size: 2.7rem; font-weight: 1000; color: #0f172a; margin-top: 5rem; border-left: 16px solid #6366f1; padding-left: 1.6rem; letter-spacing: -0.05em; }
    ...
    .vue-main-thumb { position: relative; width: 100%; height: 535px; border-radius: 45px; overflow: hidden; margin-bottom: 4.5rem; box-shadow: 0 45px 95px -15px rgba(0,0,0,0.35); }
...
const PROMPT_RULE = "MANDATORY: Return ONLY article HTML content. NO introductory filler. NO numbers in headings. Authority Magazine Style.";
...
async function run() {
    console.log("💎 VUE-ACTION-CLUSTER v1.0.4 Elite Starting...");
    ...
    const sTxt = lang === 'ko' ? "핵심 스냅샷" : "ELITE SNAPSHOT";
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const titleRaw = await callAI(model, `Unstoppable Long-tail SEO Title for: "${ctx}". NO numbers. NO "Part X". Unique and Click-worthy.`);
        const title = titleRaw.replace(/[\\"]/g, '').substring(0, 150);
        const ims = await Promise.all([1,2].map(() => genImg(title, process.env.KIE_API_KEY)));

        // Summary generation for top box and master hub
        const sumRaw = await callAI(model, `3 explosive bullet point summaries for "${title}" in ${lang}.`);
        const cleanSum = sumRaw.split("\n").filter(l => l.trim()).map(l => l.replace(/^[-*\\d.]\\s*/, '')).join('<br>');

        let body = STYLE + `<div class="vue-content-body">
            ${ims[0] ? `<div class="vue-main-thumb"><img src="${ims[0]}"><div style="position:absolute;inset:0;background:rgba(0,0,0,0.22);display:flex;align-items:center;justify-content:center;padding:30px;"><div style="font-size:3.2rem;font-weight:1050;color:#fff;text-shadow:0 8px 30px rgba(0,0,0,0.9);text-align:center;">${title}</div></div></div>` : ''}
            <div style="background:#f8fafc;border-radius:28px;padding:2.8rem;margin:4rem 0;border:2px dashed #6366f1;"><span style="font-weight:1000;color:#4338ca;font-size:1.35rem;margin-bottom:1.5rem;display:block;">${sTxt}</span>${cleanSum}</div>`;

        if (isM) {
            body += clean(await callAI(model, `Write High-Authority Master Pillar Article for "${target}" in ${lang}. NO numbers in headings. HTML ONLY.`));
            summaryHistory.forEach(s => {
                body += `<div class="vue-hub-card">
                    <span style="font-size:1.9rem;font-weight:1000;color:#1e293b;margin-bottom:1.5rem;display:block;">${s.title}</span>
                    <p style="color:#64748b;font-size:1.15rem;margin-bottom:2.2rem;">${s.summary}</p>
                    <a href="${s.url}" class="vue-btn-more">${readBtn}</a>
                </div>`;
            });
        } else {
            for(let p=1; p<=3; p++) {
                body += clean(await callAI(model, `Write Segment ${p} for article "${title}" in ${lang}. Focus on strategic depth. NO numbers in headings. HTML ONLY.`));
        ...
        const faqR = await callAI(model, `15 Professional SEO FAQs for "${title}" in ${lang}. Return ONLY valid JSON array.`);
        try {
            ...
            faqs.forEach(f => { if(f.q && f.a) fH += `<div style="margin-bottom:3.5rem;"><span style="font-weight:1000;color:#1e293b;font-size:1.6rem;display:block;margin-bottom:1rem;">Q. ${f.q}</span><p style="color:#475569;font-size:1.18rem;">${f.a}</p></div>`; });
            body += fH + `</div>`;
        } catch(e) { }

        body += `<div style="margin-top:100px;text-align:center;color:#94a3b8;font-size:13px;">© VUE Elite Platform v1.0.4</div></div>`;
